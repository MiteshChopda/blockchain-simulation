...
export default function Block({
  ...
  const handleMine = async () => {
    const { nonce: minedNonce, hash: minedHash } = await mineBlock(
      blockNo,
      data,
      previousHash,
      DIFFICULTY,
    ...
    <div className="flex flex-col rounded-sm p-4 m-auto max-w-md">
      ...
      <div className="flex flex-col gap-3 text-sm">
        ...
        <label>
          ...
          <div
            id="transaction"
            className="flex flex-col bg-amber-50 border-2 py-4 border-gray-400 rounded-xl w-full"
          >
            <Tx setData={setData} />
            <Tx setData={setData} />
          </div>
...
function Tx({ setData }: { setData: any }) {
  ...
  const handleTxChanges = async (e: React.ChangeEvent<any>) => {
    const element = e.target.id;
    const value = e.target.value;
  ...
  return (
    <div className="flex w-full justify-around items-center overflow-auto">
      Amount:&nbsp;{" "}
      <input
        id="amount"
